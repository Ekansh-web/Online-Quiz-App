<?php
  ...
  if(isset($_SESSION['key']))
  {
    if(@$_GET['q']== 'addquiz' && $_SESSION['key']=='admin') 
    {
      ...
      $wrong = $_POST['wrong'];
      $id=uniqid();
      $date = $_POST['da'].' '.$_POST['ti'].':00';
      
      
      $q3=mysqli_query($con,"INSERT INTO quiz VALUES  ('$id','$name' , '$sahi' , '$wrong','$total', NOW(),'$email','$date')");
  ...
  if(isset($_SESSION['key']))
  {
    if(@$_GET['q']== 'addqns' && $_SESSION['key']=='admin') 
    {
      ...
      for($i=1;$i<=$n;$i++)
      {
        ...
        $qns=$_POST['qns'.$i];
        $time = $_POST['tm'.$i];
        $q3=mysqli_query($con,"INSERT INTO questions VALUES  ('$eid','$qid','$qns' , '$ch' , '$i', '$time')");
        $oaid=uniqid();
        $obid=uniqid();
        $ocid=uniqid();
        $odid=uniqid();
