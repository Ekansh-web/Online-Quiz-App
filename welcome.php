...
<body>
    ...
    <nav class="navbar navbar-default title1" >
        ...
        <a class="navbar-brand" href="#"><b>Portal - Online Quiz</b></a>
        </div>
            ...
            <div class="col-md-12">
                <?php if(@$_GET['q']==1) 
                {
                    ...
                    while($row = mysqli_fetch_array($result)) {
                        ...
                        $sahi = $row['sahi'];
                        $eid = $row['eid'];
                        $admail = $row['admin_mail'];
                        $tm = $row['DD'];
                    $q12=mysqli_query($con,"SELECT score FROM history WHERE eid='$eid' AND email='$email'" )or die('Error98');
                    $rowcount=mysqli_num_rows($q12);	
                    if($rowcount == 0){
                        echo '<tr><td><center>'.$c++.'</center></td><td><center>'.$title.'</center></td><td><center>'.$total.'</center></td><td><center>'.$sahi*$total.'</center></td><td><center><b><a href="welcome.php?q=quiz&step=2&eid='.$eid.'&n=1&t='.$total.'" class="btn sub1" id="stbutt'.--$c.'" style="color:black;display: none;margin:0px;background:#1de9b6"><span class="glyphicon glyphicon-new-window" aria-hidden="true"></span>&nbsp;<span class="title1"><b>Start</b></span></a></b></center><center>by '.$admail.'</center></td></tr>';
                        echo '<h1>Quiz '.$c.' Opens in </h1><div id="DateCountdown'.$c.'" data-date="'.$tm.'" style="width: 500px; height: 125px; padding: 0px; box-sizing: border-box; background-color: #E0E8EF"></div>';
                        
                        ?>
                        <script>
                        $('#DateCountdown<?php echo $c;?>').TimeCircles();
    $('#DateCountdown<?php echo $c;?>').TimeCircles({count_past_zero: false}).addListener(countdowComplete);
	
                    function countdowComplete(unit, value, total){
                    if(total<=0){
                            $(this).fadeOut('slow').replaceWith('start the quiz');
                            document.getElementById('stbutt<?php echo $c;?>').style.display = 'inline';
                    }
                }</script> 
                    <?php
                    $c++; 
                ...
                <?php
                    if(@$_GET['q']== 'quiz' && @$_GET['step']== 2) 
                    {
                        ...
                        <div id="CountDownTimer" data-timer="'.$row['time'].'"
                            style="position:absolute; z-index: +1;width: 30%; height: 30%; margin-left:50%"></div>
                         </div>
                                 ';
                            echo '<b>Question &nbsp;'.$sn.'&nbsp;::<br /><br />'.$qns.'</b><br /><br />';
                        }
                        $q=mysqli_query($con,"SELECT * FROM options WHERE qid='$qid' " );
                        echo '<form action="update.php?q=quiz&step=2&eid='.$eid.'&n='.$sn.'&t='.$total.'&qid='.$qid.'" method="POST"  class="form-horizontal" id="fff1">
                        <br />';
    ...
    <script>
            ...
            $("#CountDownTimer").TimeCircles({count_past_zero: false}).addListener(countdownComplete);
                    ...
                    if(total<=0){
                            $(this).fadeOut('slow').replaceWith("timeup");
                            document.getElementById("fff1").submit();
                    }
                }
